...
global $wpdb;	
	...
	$listing_contact_source=get_post_meta($dir_id,'listing_contact_source',true);
	if($listing_contact_source==''){$listing_contact_source='new_value';}
	if($listing_contact_source=='new_value'){
		$client_email_address =get_post_meta($dir_id,'contact-email',true);
	}else{	
	...
	$visitor_email_address=sanitize_text_field($form_data['name']).' | ';
	$visitor_email_address=$visitor_email_address.sanitize_text_field($form_data['email_address']);
	...
	if(isset($form_data['subject'])){		
		$auto_subject=  ($form_data['subject']=="" ? $contact_email_subject : sanitize_text_field($form_data['subject'] )); 
	}else{
