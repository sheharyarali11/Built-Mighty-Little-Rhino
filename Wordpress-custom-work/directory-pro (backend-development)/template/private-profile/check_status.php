...
global $wpdb;
							...
							if(isset($recurringCheck['STATUS'])){
								if($recurringCheck['STATUS']=='Active'){
									...
									//Add  History for Payment
										...
										$my_post_form = array(
										'post_title' => wp_strip_all_tags($package_name), 
										'post_name' => wp_strip_all_tags($package_name),
										'post_content' => $total_g, 
										'post_status' => 'publish',
										'post_type' =>	$post_type,
	...
	if($payment_gateway=='woocommerce'){
		...
		if($wpayment_status=='woo_update'){ 
			 $package_id=get_user_meta($userId, 'epfuture_package_id', true);
