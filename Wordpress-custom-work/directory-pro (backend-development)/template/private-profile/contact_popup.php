...
<div class="bootstrap-wrapper" style="margin:0 20px">
		...
		<div class=" row">
			<br/>
			<h3><?php esc_html_e( 'Message Board', 'ivdirectories' ); ?></h3>	
			
		</div>
		<div class="clearfix"></div>
		<form action="" id="message-pop" name="message-pop"  method="POST" role="form">
		  <div class="form-group">
			<label for="text" class="control-label"><?php esc_html_e( 'Subject', 'ivdirectories' ); ?></label>
			<input type="text" class="form-control" id="subject" placeholder="Enter Subject">
		  </div>
		  <div class="form-group">
			<label for="text" class="control-label"><?php esc_html_e( 'Enter Message', 'ivdirectories' ); ?></label>
		  ...
		  <div class="row">
			 ...
			 <input type="hidden" name="dir_id" id="dir_id" value="<?php echo esc_html($dir_id); ?>">
			  <button type="button" onclick="send_message_iv();" class="btn btn-large btn-primary"><?php esc_html_e( 'Submit', 'ivdirectories' ); ?></button>
		...
		if (jQuery.trim(jQuery("#message-content").val()) == "") {
			...
			var loader_image = '<img src="<?php echo wp_iv_directories_URLPATH. "admin/files/images/loader.gif"; ?>" />';
				...
				var search_params={
					...
					"_wpnonce": '<?php echo wp_create_nonce("listing"); ?>',	
				};
